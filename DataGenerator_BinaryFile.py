...
import config
import numpy as np
from SamplePreprocessor import preprocess, addNoise
...
class DataGenerator:
    ...
    def createDataFiles(self):
        ...
        if config.REGENERATE_CHARLIST_AND_CORPUS:
            ...
            open(config.fnCharList, 'w', encoding="utf-8").write(str().join(localCharList))
            open(config.fnCorpus, 'w', encoding="utf-8").write(str().join(sorted(list(wordsSet))))
        ...
        lenOfValidationSamples = int(config.VALIDATION_DATASET_SPLIT_SIZE * lenOfTrainingAndValidationSamples)
    ...
    def loadDataFile(self, operationType):
        ...
        for line in f:
            ...
            if operationType == config.OperationType.Training:
                self.trainSamples.append(Sample(gtText, imgIdx, imgHeight, imgWidth, imgSize, imgStartPosition))
            elif operationType == config.OperationType.Validation:
                self.validationSamples.append(Sample(gtText, imgIdx, imgHeight, imgWidth, imgSize, imgStartPosition))
            elif operationType == config.OperationType.Testing:
                self.testSamples.append(Sample(gtText, imgIdx, imgHeight, imgWidth, imgSize, imgStartPosition))
    ...
    def selectValidationSet(self):
        ...
        self.samples = self.validationSamples[:config.VALIDATIOIN_SAMPLES_PER_STEP]
    ...
    def getNext(self):
        ...
        for i in batchRange:
            try:
                ...
                img = np.frombuffer(self.binaryImageFile.read(self.samples[i].imageSize), np.dtype('B'))
                img = img.reshape(self.samples[i].imageHeight, self.samples[i].imageWidth)
                img = preprocess(img, config.IMAGE_WIDTH, config.IMAGE_HEIGHT, config.RESIZE_IMAGE,config.CONVERT_IMAGE_TO_MONOCHROME, config.AUGMENT_IMAGE)
                imgs.append(img)
            except IOError as e:
