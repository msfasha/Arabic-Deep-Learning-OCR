...
import config
from config import DecoderType
...
class Model:
    ...
    def __init__(self, decoderType=DecoderType.BestPath, mustRestore=False, dump=False):
        ...
        # input image batch
        self.inputImgs = tf.placeholder(tf.float32, shape=(None, config.IMAGE_WIDTH, config.IMAGE_HEIGHT))
    ...
    def auditModelDetails(self):
        ...
        for variable in tf.trainable_variables():
            ...
            saveString = saveString + "Shape:" + str(shape) + " ,shape length:" + str(len(shape))
            variable_parameters = 1
            for dim in shape:
                variable_parameters *= dim.value
            saveString = saveString + " , parameters: " + str(variable_parameters) + "\n"
            total_parameters += variable_parameters

        saveString = saveString + "Total Parameters: " + str(total_parameters) + "\n\n"
    ...
    def setup5LayersCNN(self):
        ...
        self.kernel1 = tf.Variable(tf.truncated_normal([5, 5, 1, 32], stddev=0.1))
        self.conv1 = tf.nn.conv2d(pool, self.kernel1, padding = 'SAME', strides = (1, 1, 1, 1))
        conv_norm = tf.layers.batch_normalization(self.conv1, training=self.is_train)
        self.relu1 = tf.nn.relu(conv_norm)
        self.pool1 = tf.nn.max_pool(self.relu1, (1, 2, 2, 1), (1, 2, 2, 1), 'VALID')

        kernel = tf.Variable(tf.truncated_normal([5, 5, 32, 64], stddev=0.1))
        self.conv2 = tf.nn.conv2d(self.pool1, kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv_norm = tf.layers.batch_normalization(self.conv2, training=self.is_train)
        relu = tf.nn.relu(conv_norm)
        pool = tf.nn.max_pool(relu, (1, 2, 2, 1), (1, 2, 2, 1), 'VALID')

        kernel = tf.Variable(tf.truncated_normal([3, 3, 64, 128], stddev=0.1))
        self.conv3 = tf.nn.conv2d(pool, kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv_norm = tf.layers.batch_normalization(self.conv3, training=self.is_train)
        relu = tf.nn.relu(conv_norm)
        pool = tf.nn.max_pool(relu, (1, 1, 2, 1), (1, 1, 2, 1), 'VALID')

        kernel = tf.Variable(tf.truncated_normal([3, 3, 128, 128], stddev=0.1))
        self.conv4 = tf.nn.conv2d(pool, kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv_norm = tf.layers.batch_normalization(self.conv4, training=self.is_train)
        relu = tf.nn.relu(conv_norm)
        pool = tf.nn.max_pool(relu, (1, 1, 2, 1), (1, 1, 2, 1), 'VALID')

        kernel = tf.Variable(tf.truncated_normal([3, 3, 128, 256], stddev=0.1))
        self.conv5 = tf.nn.conv2d(pool, kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv_norm = tf.layers.batch_normalization(self.conv5, training=self.is_train)
        relu = tf.nn.relu(conv_norm)
        pool = tf.nn.max_pool(relu, (1, 1, 2, 1), (1, 1, 2, 1), 'VALID')
    ...
    def setupCNN7Layers(self):
        ...
        conv1 = tf.nn.conv2d(cnnIn4d, kernel1, padding = 'SAME', strides = (1, 1, 1, 1))
        ...
        kernel3 = tf.Variable(tf.truncated_normal([3, 3, 128, 256], stddev=0.1))
        conv3 = tf.nn.conv2d(pool2, kernel3, padding='SAME', strides=(1, 1, 1, 1))

        kernel4 = tf.Variable(tf.truncated_normal([3, 3, 256, 256], stddev=0.1))
        conv4 = tf.nn.conv2d(conv3, kernel4, padding='SAME', strides=(1, 1, 1, 1))
        pool3= tf.nn.max_pool(conv4, (1, 1, 2, 1), (1, 1, 2, 1), 'VALID')

        kernel5 = tf.Variable(tf.truncated_normal([3, 3, 256, 512], stddev=0.1))
        conv5 = tf.nn.conv2d(pool3, kernel5, padding='SAME', strides=(1, 1, 1, 1))
        batch_norm1 = tf.layers.batch_normalization(conv4, training=self.is_train)

        kernel6 = tf.Variable(tf.truncated_normal([3, 3, 512, 512], stddev=0.1))
        conv6 = tf.nn.conv2d(batch_norm1, kernel6, padding='SAME', strides=(1, 1, 1, 1))
        batch_norm2 = tf.layers.batch_normalization(conv6, training=self.is_train)
        pool4 = tf.nn.max_pool(batch_norm2, (1, 1, 2, 1), (1, 1, 2, 1), 'VALID')

        kernel7 = tf.Variable(tf.truncated_normal([2, 2, 512, 512], stddev=0.1))
        conv7 = tf.nn.conv2d(batch_norm1, kernel7, padding='SAME', strides=(1, 1, 1, 1))
    ...
    def setupCTC(self):
        ...
        self.savedCtcInput = tf.placeholder(tf.float32, shape=[config.MAX_TEXT_LENGTH, None, len(self.charList) + 1])
    ...
    def setupTF(self):
        ...
        saver = tf.train.Saver(max_to_keep=config.MAXIMUM_MODELS_TO_KEEP)  # saver saves model to file
...
#         ######################################################
    ...
    def save(self):
        "save model to file"
        self.snapID += 1
        self.saver.save(self.sess, config.MODEL_PATH + config.EXPERIMENT_NAME, global_step=self.snapID)
